...
Это одно из основных нововведений. Теперь по умаолчанию при инициализации элеметов вместо `DefaultElementLocator` 
будет использоваться `AjaxElementLocator`. Это позволяет производить поиск элементов при обращении к ним не сразу же, 
а с задержкой (в 5 секунд). Из названия `Ajax...` понятно для чего это нужно.
