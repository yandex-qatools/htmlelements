...
Мы поправили несколько недочетов архитектуры (`Checkbox`, `AjaxElementLocator`) и пару ошибок, связанных с инициализацией 
`List<Т extends HtmlElements>`.

Также у нас появились контрибьюторы! Огромное спасибо [@lanwen](http://github.com/lanwen) за отличную реализаю идеи 
WaitFor-матчера. И, хотя один pull-реквест у нас еще не принят, авансом спасибо [@pazone](http://github.com/pazone) за 
идею матчера для рекурсивной проверки налчиия на странице внутренних элементов блока. Надеемся, что он появится в 
следующей версии.
...
Это одно из основных и очень важных нововведений. Теперь по умаолчанию при инициализации элеметов вместо 
`DefaultElementLocator` будет использоваться `AjaxElementLocator`. Это позволяет производить поиск элементов при обращении 
к ним не сразу же, а с задержкой (в 5 секунд). Из названия `Ajax...` понятно для чего это нужно.
...
С ростом количества JavaScript логики на клиенсткой стороне, в тестах все чаще требуется использовать различные таймауты.<br/>
...
В предыдущей версии при инициализации списка HtmlElement-ов (или TypifiedElement-ов) из-за неудачного способа именования 
элементов списка происходило сравнение каждого элемента колекции с каждым.В результате работа с со списками существенно 
притормаживала.<br/>
В этой версии мы поменяли алгортим именования элементов списка и теперь все работает отлично.
...
По-этому мы добавили эти методы в базовый класс TypifiedElement.
...
В прошлой версии при реализации Checkbox'а бы допущена архитектураная ошибка:
...
А сейчас пора бежать спасать мир :)
