...
which recursively checks that elements of a block exsist on a page, though the pull-request is still unaccepted. 
...
This is one of the main and most important features of this release. Now by default `AjaxElementLocator` will be used for 
initialization of elements instead of `DefaultElementLocator`. It allows to search element not immediately after its 
method call but after a time-out has expired (5 seconds by default). The name `Ajax...` explains its destination.
...
We've also added the ability to use your own implementation of `Element Locator` for elements initialization:
