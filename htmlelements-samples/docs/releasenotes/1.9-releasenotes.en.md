...
sent to a server asynchronously, but the result of its execution comes into effect only after a page reload. Or may need
to check that some page element changes after every refresh (e.g. an advertising banner, captcha, etc.).<br/>
